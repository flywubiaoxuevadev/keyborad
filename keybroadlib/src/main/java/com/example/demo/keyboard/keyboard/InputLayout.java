...
public class InputLayout extends RelativeLayout implements OnStockAmountListener, KeyboardView.OnKeyboardActionListener {
    ...
    private void initView() {
        inflate(getContext(), R.layout.keybroad_lv, this);
        keyboardViewNumber = findViewById(R.id.keyboard_number);
        keybroad_tv= (TextView)findViewById(R.id.keybroad_tv);
    ...
    private void hide() {
        ...
        keybroad_tv.setVisibility(GONE);
     ...
     */
    public void initKeyboardView(int keyboardType) {
        isCap = false;
...
//                keyboardViewNumber.setOnKeyboardActionListener(this);
            ...
            case Constants.KEYBOARD_TYPE_NUMBER_ALL:
                ...
                keyboardViewNumber.setVisibility(VISIBLE);
                keybroad_tv.setVisibility(VISIBLE);
...
//                keyboardViewNumber.setKeyboard(keyboardNumberAll);
    ...
    /**
     *解决左右行数不对等的问题，同时设置线性布局长为股票键盘的/5，高度和股票键盘一致
     */
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        ...
        if (primaryCode == Constants.KEYCODE_DELETE) {
            if (editText != null) {
                ...
                //删除前一个字符
                mOnOKOrHiddenKeyClickListener.onDeleteCharest();
            }
            ...
            if (editText != null) {
                ...
                //传递回调的内容
                mOnOKOrHiddenKeyClickListener.onGetDataClick(Character.toString((char) primaryCode));
            }
        }
        //清除键
        if (primaryCode == Constants.KEYCODE_CLEAR) {
            if (editText != null) {
                ...
                //删除所有字符
                mOnOKOrHiddenKeyClickListener.clearAllCharest();
            }
        }
        //确认键
        if (primaryCode == Constants.KEYCODE_OK) {
            if (editText != null) {
                ...
                mOnOKOrHiddenKeyClickListener.submitCharest();
            }
